...
1. 输出："§2.4.1 开始验收 Pre-Existing Failure。我不执行任何命令，只分析 sub-agent 的汇报。"
2. 检查 sub-agent 汇报中是否存在任何测试失败，输出："§2.4.2 wave <id> 是否有 Pre-Existing Failure：<YES/NO>"
3. 如果 §2.4.2 = YES，输出："§2.4.3 有 Pre-Existing Failure。我将判断用户是否说过可以忽略。我只看用户说没说过，不考虑这些问题是否是新引入的。"
4. 检查用户是否在本轮对话中明确说过可以忽略，输出："§2.4.4 用户 <明确说过/没有说过> 可以忽略这些 Pre-Existing Failure"
5. 如果 §2.4.2 = YES 且 §2.4.4 = 没有说过，输出："§2.4.5 判定：不合规。立即派发修复。" 然后回到 §2.3 派发新 sub-agent 修复。**§2.4.4 输出后必须立即输出 §2.4.5，中间不得执行任何命令、不得向用户提问、不得做额外验证。**
