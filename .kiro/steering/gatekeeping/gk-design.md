...
如果 `graphify_ready`，优先使用 graphify 子命令进行结构化查询，辅助后续 Impact Analysis 和技术方案质量校验。
...
如果包含 `## Correctness Properties`，每条 property 必须使用 `### Property N: <名称>` 格式，包含 Validates（格式 `Requirements X.Y, X.Z`，X = Requirement 编号，Y = AC 编号，可省略 .Y 仅引用 Requirement 级别）、条件、保证、违反后果四项。
...
- [ ] 如果 `graphify_ready`，是否利用 graphify 辅助识别了受影响范围
...
- [ ] **Requirements CR 回应**：用户在 requirements GK CR 中的决策是否在 design 中体现。
...
**不应涉及**：行为层面的 trade-off（属于 requirements 阶段）、非功能约束的有无（属于 requirements 阶段）。
