...
review 过程中如果在 diff 上下文（非本次变更的行）或未列入本次 diff 的代码中发现 bad smell：

1. **有注释说明原因**（如既有技术债说明或 `[review-skip]`）→ 放过，不报告
2. **无注释说明** → 按上文 **RED → GREEN TDD** 直接修复；不得以范围或引入时间推诿。若用户事后明确表示不处理某处，再于该处添加 `// [review-skip] <原因>` 类注释供后续 review 放过

不得以"不是本次迭代改的"或"不在扫描文件列表里"为由跳过无注释的问题。
