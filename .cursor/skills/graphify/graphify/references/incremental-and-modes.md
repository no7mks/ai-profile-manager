...
Use this file when the user asks to refresh or reorganize an existing graph without running a full first-time flow.

Command intent:
- `--update`: re-extract only changed/new files since last manifest
- `--cluster-only`: skip extraction and re-run clustering/reporting

Shared rules:
- Keep previous graph data safe during update (`.graphify_old.json` backup before merge).
- If no changed files are detected on `--update`, stop with a clear "nothing to do" message.
- For code-only changes, skip semantic extraction (no LLM cost).
- After mode execution, regenerate user-facing output artifacts and report graph delta when possible.
