...
Use this file for full build execution (`/graphify <path>`) and as the base flow reused by `--update` (when semantic extraction is needed).

Before running steps:
- If no path is provided, use `.`
- Keep a single interpreter source of truth in `graphify-out/.graphify_python`
- Respect stop conditions early (`total_files == 0`, empty graph, oversized corpus decision gate)
- Keep extraction honest: explicit confidence tags and no fabricated edges

At the end of flow:
- Always summarize output artifacts and token/cost information
- Paste only key report sections (God Nodes, Surprising Connections, Suggested Questions)
- Offer one high-value follow-up exploration question
