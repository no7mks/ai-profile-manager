...
tasks.md 的 top-level task 遵循**固定顺序**：
...
文档收敛是倒数第二个 top-level task（Code Review 之前），必须包含：

- state 文档更新 sub-task（与 design Impact Analysis 一致）
- manual 文档更新 sub-task（如适用）
- migration guide sub-task（如适用）
- checkpoint sub-task

## Code Review task 要求

- 描述为"委托给 code-reviewer sub-agent 执行"
- 不展开 review checklist（由 code-reviewer 自行决定）
...
每个 top-level task 的最后一个 sub-task 必须是 checkpoint：

```markdown
- [ ] 1.N Checkpoint
  - 运行 `<具体验证命令>`（如 `./vendor/bin/phpunit`）
