...
3. 输出："角色确认：<Main-agent / Sub-agent>，进入 Step <2/3>。"
...
1. 输出："开始 Drift 检测，比对当前代码与 design.md 预期……"
...
4. 输出："2.1 完成。Drift 检测结果：<无 drift / 有 drift 但不影响 / 有 drift 需确认>"
...
5. **分析并行性**：若 wave 内包含不止一个任务，按 [references/orchestration.md](references/orchestration.md) 的并行策略分析所有任务是否可以并行执行
6. **严格避免**：执行非范围内的任务。
7. 输出："2.2 完成。执行范围：<wave/task 列表>，执行方式：<串行/并行>"

### 2.3 执行任务

1. 输出："不管任务多少、难度几何，都不允许在 main-agent 执行任务！开始派发 sub-agent……"
2. 按照 2.2 的分析，为每个 sub-task 派发一个 sub-agent 执行，串行的任务等上一个 sub-task 完毕后再派发下一个。对每个 sub-task 派发 sub-agent 时，必须包含以下上下文：
...
4. 输出："2.3 完成。所有 sub-agent 已返回结果。"
...
### 2.5 Pre-existing Failure Review

1. 输出："开始审查 pre-existing failure 合规情况……"
2. 审查每个 sub-agent 的完成汇报，确认是否存在 pre-existing failure
...
6. 输出："2.5 完成。Pre-existing failure 审查结果：<无 / 已合规处理 / 已派发修复>"

### 2.6 汇总与推进

1. 输出："开始汇总执行结果……"
2. 并行组内所有 sub-agent 完成后，main-agent review 结果
3. 标记 tasks.md 进度
4. **硬停止**：完成当前 top-level task 或 wave 后，**禁止**自动执行下一个 task/wave。不得以"让我继续下一个"、"接下来执行"等措辞自行推进。必须停下来，等待用户显式发出下一步指令（如 next wave、next task、继续执行）。标记完成后的唯一允许动作是向用户汇报当前进度。
5. 输出："2.6 完成。当前进度：<已完成的 task/wave>。等待用户指令。"
...
3. 输出："3.1 完成。已加载执行模型，task 类型判断为 <task-type>。"
...
3. 输出："3.2 完成。拆分为 <N> 个 sub-steps：<列表>。"
