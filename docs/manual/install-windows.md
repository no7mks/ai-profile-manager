...
在 Windows（PowerShell + Scoop Composer）上安装 apm CLI 并完成两阶段首装的操作说明。命令行为 SSOT 见 `docs/state/cli-commands.md`。
...
`bootstrap` 执行两步操作：
1. 搭建项目 scaffold（`docs/`、`issues/`、`AGENTS.md` 等）
2. 安装 `abilities.yaml` 中 `bootstrap.includes` 列表定义的 ability 到项目目录
