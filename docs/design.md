...
以下路径为**权威约定**；`Installer`、`CaptureService` diff、`CaptureWriteBackService` ingest 均须与此一致。文中 `<cwd>` 表示执行 CLI 时的当前工作目录：安装能力时为**业务仓库根**；ingest write-back 时为 **aipm-repo 根**（集中收录 baseline 的仓库）。`<target>` 为 `cursor` 或 `kiro`。

### Skill

| 场景 | 路径 |
|------|------|
| 包内 / aipm-repo 中 SSOT | `abilities/skills/<name>/**`（整棵目录；**不**再按 `<name>/<target>/` 分平台子目录） |
...
| 场景 | 路径 |
|------|------|
| 包内 / aipm-repo 中 SSOT | `abilities/agents/<name>.<target>.md`（**扁平**单文件；每个 target 一份源文件） |
...
| 场景 | 路径 |
|------|------|
| 包内 / aipm-repo 中 SSOT | `abilities/rules/<rule-relative-path>/<rule-name>.<target>.<ext>`，其中 `<rule-relative-path>` 可为多层子目录或为空（文件直接在 `abilities/rules/` 下）；`<ext>` 支持 **cursor**：`.cursor.mdc`、`.cursor.md`；**kiro**：`.kiro.md`、`.kiro.mdc`。安装与 capture 在**同一 basename** 多后缀时优先 `.cursor.mdc` / `.kiro.md`。 |
...
- Manifest：`abilities/_presets.json`。ingest 时 `type: preset` 的 `files[].path` 为相对 **aipm-repo 仓库根**（例如 `abilities/_presets.json`），由 `CaptureWriteBackService` 拼接到当前目录，**不**走 `abilities/skills/...` 等前缀规则。
...
- `CaptureService`：capture、change 组装与落盘（`~/.aipm/changes`）；diff 侧 SSOT 路径与 **「能力路径约定」** 一致。
...
- `CaptureWriteBackService`：将 change 写回 `aipm-repo` 文件（含删除与 `preset` manifest）；各 `type` 的落盘路径见 **「能力路径约定」** 中 Capture / ingest 列。
